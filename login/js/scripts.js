    ...
    var handleAllJQVMAP = function () {
        ...
        var data = {
                ...
                onRegionClick: function (element, code, region) {
                    ...
                    $.getJSON("/api/zone",function(data,status){
                        ...
                        var message = region + '：' + zone_state_temperature + '（' + zone_temperature + '℃），' + zone_state_humidity + '（' + zone_humidity + '%）。';
                        alert(message);
        ...
        function countSecond( )
        {
            ...
            $.getJSON("/api/sensor",function(data,status){
                ...
                {
                    flag_pro = '温度过高';
                    map.vectorMap('set','colors',{'aa':'red'});
                    flag_zone=1;
                }
                else if(zone_temperature <25)
                {
                    flag_pro = '温度过低';
                    map.vectorMap('set','colors',{'aa':'green'});
                    flag_zone=1;
                }
                else {
                    map.vectorMap('set','colors',{'aa':'#c6c6c6'});
                }
                if(zone_humidity > 70)
                {
                    flag_pro = '湿度过高';
                    map.vectorMap('set','colors',{'aa':'blue'});
                    flag_zone=1;
                }
                else if(zone_humidity <40)
                {
                    flag_pro = '湿度过低';
                    map.vectorMap('set','colors',{'aa':'yellow'});
                    flag_zone=1;
                }
                else {
                    map.vectorMap('set','colors',{'aa':'#c6c6c6'});
            ...
            if(flag_pro.length==1)
                flag_repeat=1;
            if(flag_repeat==1&&flag_pro.length!=1)
            {
            flag_repeat=0;
            var message =  flag_zone+'区:'+ flag_pro + '\n' + '结合当前天气情况，建议对此区进行灌溉操作。';
            alert(message);
          // $("#spId1").append('<span>温度过低</span>');
           //$("#spId2").text("刚刚"); 
           $("#ulId").append('<li id="liId"></li>');
           $("#liId").append('<span class="label label-important" id="spId0"></span>');
           $("#spId0").append('<i class="icon-bell"></i>');
           $("#liId").append('<span id="spId1">'+message+'</span> ');
           $("#liId").append('<span class="small italic" id="spId2"></span> ');
           $("#spId2").text("刚刚");
            }
            if(flag_zone == 0) {
                map.vectorMap('set','colors',{'aa':'#c6c6c6'});
        ...
        var t1 = window.setInterval(countSecond,500); 
    }
...
/*         var pageviews = [
        ...
        $("#site_statistics").bind("plothover", function (event, pos, item) {
            ...
            if (item) {
                if (previousPoint != item.dataIndex) {
                    ...
                    showTooltip(item.pageX, item.pageY, x + "日的"+item.series.label + " 为 " + y + " ℃ " );
                }
            } else {
                $("#tooltip").remove();
                previousPoint = null;
        ...
        var plot = $.plot($("#load_statistics"), [getRandomData2()], options);
